...
class TM_Helpers
{
    ...
    public static function get_listing_statuses(): array
    {
        return array(
            ...
            'expired' => __('Expirado', 'terramarket'),
        );
    }

    public static function get_user_selectable_listing_statuses(): array
    {
    ...
    public static function get_listing_raw_status(int $listing_id): string
    {
        $status = (string) get_post_meta($listing_id, 'tm_listing_status', true);

        return array_key_exists($status, self::get_listing_statuses()) ? $status : 'active';
    }

    public static function get_listing_expiration_date(int $listing_id): string
    {
        return (string) get_post_meta($listing_id, 'tm_expiration_date', true);
    }

    public static function should_treat_status_as_expired(string $status, string $expiration_date): bool
    {
        if ('active' !== $status || '' === $expiration_date) {
            return false;
        }
    ...
    public static function is_listing_expired(int $listing_id): bool
    {
        return self::should_treat_status_as_expired(
            self::get_listing_raw_status($listing_id),
            self::get_listing_expiration_date($listing_id)
        );
    }

    public static function get_effective_listing_status(int $listing_id): string
    {
        $status = self::get_listing_raw_status($listing_id);

        return self::is_listing_expired($listing_id) ? 'expired' : $status;
    }

    public static function get_default_listing_expiration_date(int $days = 30): string
    {
        $days = max(1, $days);
    ...
    public static function reset_listing_expiration_date(int $listing_id, int $days = 30): string
    {
        $expiration = self::get_default_listing_expiration_date($days);
        update_post_meta($listing_id, 'tm_expiration_date', $expiration);

        return $expiration;
    }

    public static function ensure_listing_expiration_date(int $listing_id, int $days = 30): string
    {
        $expiration = self::get_listing_expiration_date($listing_id);
        if ('' !== $expiration) {
            return $expiration;
        }

        return self::reset_listing_expiration_date($listing_id, $days);
    }

    public static function get_public_active_listings_meta_query(): array
    {
        $today = date('Y-m-d', current_time('timestamp'));
    ...
    public static function hex_to_rgba(string $hex_color, float $alpha = 1): string
    {
        $hex = ltrim(self::sanitize_hex($hex_color, '#ffffff'), '#');
        if (3 === strlen($hex)) {
